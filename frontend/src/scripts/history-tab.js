/**
 * history-tab.js
 * Lógica del tab Historial (dh): lista, filtros, detalle de análisis
 * y panel de recomendaciones dentro del detalle.
 */
...
export function getAnalysisUserNum() { return _analysisUserNum }
...
function _histTableRow(a) {
  ...
  return `
    <tr class="border-b border-sand hover:bg-sand/20 transition-colors">
      ...
      <td class="px-4 py-3.5">
        <button onclick="exportAnalysisPdf(${a.id})"
          title="Exportar análisis en PDF"
          class="w-8 h-8 rounded-full hover:bg-sand flex items-center justify-center text-slate hover:text-forest transition-colors">
          <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 10v6m0 0l-3-3m3 3l3-3M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2"/>
            <path d="M8 6H6a2 2 0 00-2 2v2M16 6h2a2 2 0 012 2v2M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M8 6h8"/>
          </svg>
        </button>
      </td>
    </tr>`
  ...
  return `
    <div class="bg-white rounded-2xl border border-sand p-4">
      ...
      <div class="flex gap-2">
        <button onclick="openDetail(${a.id})"
          class="flex-1 bg-forest text-white text-[12px] font-semibold py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-forest/90 transition-colors">
          <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
            <circle cx="12" cy="12" r="3"/>
          </svg>
        ...
        <button onclick="exportAnalysisPdf(${a.id})"
          title="Exportar PDF"
          class="w-11 bg-sand/60 text-slate rounded-xl flex items-center justify-center hover:bg-sand hover:text-forest transition-colors flex-shrink-0">
          <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 10v6m0 0l-3-3m3 3l3-3M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2"/>
            <path d="M8 6H6a2 2 0 00-2 2v2M16 6h2a2 2 0 012 2v2M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M8 6h8"/>
          </svg>
...
export function populateHistoryTab(analyses) {
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-sand/40 border-b border-sand">
            ...
            <th class="px-4 py-3 text-[10px] font-semibold text-slate uppercase tracking-wider">PDF</th>
          </tr>
        </thead>
        <tbody id="hist-tbody">
...
function _attachHistoryFilters(allAnalyses) {
  function applyFilters() {
    ...
    if (tbody) tbody.innerHTML = filtered.length
      ...
      : `<tr><td colspan="9" class="py-8 text-center text-xs text-slate">No hay análisis que coincidan.</td></tr>`
